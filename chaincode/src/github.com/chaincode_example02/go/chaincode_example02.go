...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "initUsers" {
		return t.initUsers(stub, args)
...
func (t *SimpleChaincode) initUsers(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// Write the state to the ledger
	err = stub.PutState("users", []byte(usersJson))
	if err != nil {
		return shim.Error(err.Error())
	}
	return shim.Success(nil)
...
func (t *SimpleChaincode) addUser(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	usersJson, _ := stub.GetState("users")
...
func (t *SimpleChaincode) queryUsers(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	Avalbytes, err := stub.GetState("users")
	if err != nil {
	...
	if Avalbytes == nil {
		jsonResp := "{\"Error\":\"Nil Value for " + "users" + "\"}"
		return shim.Error(jsonResp)
	}

	jsonResp := "{\"Name\":\"" + "users" + "\",\"Value\":\"" + string(Avalbytes) + "\"}"
