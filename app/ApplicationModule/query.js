...
var getBlockByNumber = async function(peer, channelName, blockNumber, username, org_name) {
	try {
		...
		if (response_payload) {
			let result = {};
			result.header = response_payload.header;
			result.tx_id = response_payload.data.data[0].payload.header.channel_header.tx_id
			result.timestamp = response_payload.data.data[0].payload.header.channel_header.timestamp
			logger.debug(result);
			return result;
		} else {
...
var getChannels = async function(peer, username, org_name) {
	try {
		...
		let peerResponse = await channel.queryInfo(peer)
		if (response) {
			logger.debug('<<< channels >>>');
			logger.debug("test",peerResponse)
			var channelNames = [];
...
var getBlockHeight = async (peer, channelName,username, org_name) =>{
	try {
		// first setup the client for this org
		var client = await helper.getClientForOrg(org_name, username);
		logger.debug('Successfully got the fabric client for the organization "%s"', org_name);
		...
		let peerResponse = await channel.queryInfo(peer)
		if (peerResponse) {
			logger.debug('<<< Blocj Data >>>');
			logger.debug(peerResponse.height.low);
			return {result:peerResponse.height.low};
		} else {
			logger.error('peer Response is null');
			return 'peer Response is null';
		}
	} catch(error) {
		logger.error('Failed to query due to error: ' + error.stack ? error.stack : error);
		return error.toString();
